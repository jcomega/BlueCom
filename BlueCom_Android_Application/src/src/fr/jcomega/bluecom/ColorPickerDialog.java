...
public class ColorPickerDialog extends Dialog {
    ...
    private static class ColorPickerView extends View {
        ...
        ColorPickerView(Context c, OnColorChangedListener l, int color, int alpha) {
            ...
            mCenterPaint.setStrokeWidth(LITTLESTROKEWIDTH);
            ...
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    if (mTrackingCenter) {
                        ...
                        downPaint.setColor(interpColor(mColors, unit));
                        invalidate();
                    }
                    else{
                    	if (event.getX() < CENTER_X/5)
                    	{
                    		mCenterPaint.setAlpha(0);	//alpha = min
                    	}
                    	else if (event.getX() > ((CENTER_X*2)-CENTER_X/5))
                    	{
                    		mCenterPaint.setAlpha(255);	//alpha = max
                    	}
                    	else
                    	{
                        	float alpha;
                    		...
                    		alpha = ((255 * event.getX()) / ((CENTER_X*2)-CENTER_X/5));
                    		mCenterPaint.setAlpha((int) alpha);
                    	}
    ...
    public ColorPickerDialog(Context context,
                             ...
                             int initialColor, int initialAlpha) {
        super(context);
